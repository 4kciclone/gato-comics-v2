...
export default async function ExplorePage() {
    ...
    // Buscas em paralelo para os posts mais curtidos e usuários mais seguidos
    const [popularPostsData, influentialUsers] = await Promise.all([
        prisma.post.findMany({
            where: { parentId: null },
            orderBy: {
                likes: { _count: 'desc' } // Ordena por número de curtidas
            },
            take: 20,
            include: {
                user: { select: { id: true, name: true, image: true, equippedAvatarFrame: { select: { imageUrl: true } } } },
        ...
        prisma.user.findMany({
            orderBy: {
                followers: { _count: 'desc' } // Ordena por número de seguidores
            },
            take: 5,
            select: {
                id: true,
            ...
            <div className="p-4 border-b border-[#27272a]">
                ...
                <div className="flex gap-4 overflow-x-auto scrollbar-hide">
                    {influentialUsers.map(user => (
                        <Link key={user.id} href={`/u/${user.name}`} className="shrink-0">
