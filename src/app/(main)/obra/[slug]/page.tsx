    ...
    <div className="min-h-screen bg-[#050505]">
      ...
      <div className="relative w-full min-h-[80vh] md:h-auto md:min-h-[600px] flex items-center pt-24 pb-12 overflow-hidden">
        ...
        <div className="container mx-auto px-4 h-full relative z-10 flex flex-col items-center text-center md:flex-row md:items-end md:text-left gap-8">
          ...
          <div className="w-48 md:w-64 aspect-2/3 rounded-xl overflow-hidden shadow-2xl border-2 border-zinc-800 shrink-0 md:transform md:translate-y-16 shadow-black/50">
            <WorkImage src={work.coverUrl} alt={work.title} className="w-full h-full" />
          </div>

          <div className="flex-1 space-y-4">
             <div className="flex flex-wrap items-center justify-center md:justify-start gap-2">
                <Badge variant="secondary" className="bg-[#FFD700] text-black font-bold">Webtoon</Badge>
                {work.genres.slice(0, 3).map(g => (<Badge key={g} variant="outline" className="border-zinc-700 backdrop-blur-sm bg-black/20">{g}</Badge>))}
             </div>
             
             <h1 className="text-4xl sm:text-5xl md:text-7xl font-black text-white tracking-tighter uppercase drop-shadow-lg">{work.title}</h1>
             
             <div className="flex items-center justify-center md:justify-start flex-wrap gap-x-6 gap-y-2 text-zinc-300 font-medium">
      ...
      <div className="container mx-auto px-4 mt-16 md:mt-24 space-y-24">
         <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
            <aside className="lg:col-span-4 space-y-8">
               <div className="bg-[#111111] border border-[#27272a] rounded-2xl p-6 lg:sticky lg:top-24">
                  <h3 className="text-xl font-bold text-white mb-6">Sobre a Obra</h3>
                  <div className="space-y-6">
                     <div className="space-y-2"><span className="text-xs font-bold text-zinc-500 uppercase tracking-wider">Sinopse</span><p className="text-zinc-300 leading-relaxed text-sm">{work.synopsis}</p></div>
